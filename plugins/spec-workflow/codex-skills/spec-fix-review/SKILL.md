...
description: |
  将 review 文档发送到指定 tmux pane，让另一个 AI agent 先调用 spec-workflow:spec-check-review
...
5. 生成修复指令消息（末尾附带反馈指令）
6. 通过 tmux:tmux-send skill 发送（由 tmux:tmux-send 负责确定目标 pane）
...
```
使用 spec-workflow:spec-check-review skill 查看以下 review 文档，并完成修复：

{review 文档路径}

---

执行完成后，调用 spec-workflow:spec-feedback skill 向 pane {当前pane_id} 反馈结果。
...
使用 tmux:tmux-send skill 发送，由它负责处理目标 pane 的选择。
