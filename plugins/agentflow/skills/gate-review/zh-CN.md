...
4. 输出审查结论
5. 决定下一步：
   - 需修复 → 输出问题列表和修复建议，由用户决定要不要发回去。只有当用户此前已经说过「一直循环别问我」时，才自行重派。
   - 通过 → 标记完成
...
| 需修复 | 输出问题列表和修复建议，由用户决定要不要发回去 |
| 接收端拒绝有理 | 接受拒绝，调整任务或标记完成 |
| 接收端拒绝无理 | 告知用户，由用户决定是否附反驳重发 |

默认把决定权交回去，而不是自行重派。一次错误的自动重做要花掉对方 agent 一整轮工作，还可能把原本的交付物埋在没人要的改动底下；而问一句只要一行。只有当用户此前已经说过「一直循环别问我」时，才直接用 `tmux_dispatch` 重派。
