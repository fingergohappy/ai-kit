...
│   ├── CR-NNN-<slug>.md      #   项目 PR 模板的副本, 实施当中逐条用中文填, 开 PR 时翻成英文贴进正文
...
| 文件 | 是什么 | 谁读它 |
|---|---|---|
| `release/CR-NNN-<slug>.md` | 项目 PR 模板的整份副本 (`sdd.py new-spec` 时建), 实施当中逐节用中文填事实 | 开 PR 时翻成英文整份贴进正文 (`ship-pr`), 之后是发布清单的来源 (`release-ops`) |
...
**语言: release 文件中文, PR 正文英文.** 这份文件与 REQ / CR / spec 一样按写作约定用中文正文
(ASCII 标点); 模板自带的节标题、表头、检查项题头保留原文, 引用的命令输出、错误文本、标识符、
路径与 hash 逐字不动. 项目的 PR 模板是英文的, 照着英文骨架填空很容易顺手写成英文 -- 那是
...
- 中文正文, ASCII 标点; 代码, 接口, 字段名保持原文. `release/` 下的上线产物也是中文 (模板的
  节标题与表头除外); 只有开 PR 时贴进正文的那份译成英文, 见 "上线产物" 一节.
