...
**工作树只能有一个人改.** impl 阶段的车道 E 要做变异验证 (改一行看红不红; 只改 diff 里的行, 只跑相关用例, 做法见 review-lanes.md),
...
- 审什么: CR / REQ / spec 的**绝对路径**, impl 阶段另给基点 commit 与 `git diff` 范围 (车道 E 的变异只打这个范围里的行)
