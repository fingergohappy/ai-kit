...
- **审查**——**发现是主张，不是事实**。合并之前先自己核：那个 `file:line` 真的在吗，代码真的那样写吗，它引用的规则原文真是那个意思吗。核不实的剔除并留痕（说清剔了几条、为什么），一时核不动的降级并标「未核实」。照单全收的代价是你会照着一条假发现去改一段本来正确的代码。核完再合进一份文件：车道之间有重叠时保留更锋利的那条表述，不要取平均。**整体结论取最严的那个车道**：只要有一条 BLOCK，整份 review 就是 BLOCK，不管另外几条报了多少个 OK。
...
**审查形态要晚一步收。** 一份 review 不是审完就结束了：发现要拿去修，修完还得让**报出它的那个
agent** 复核（换个人复核得重读一遍代码才有同样的判断，而改代码的人复核自己的修复，等于既当
运动员又当裁判）。所以审查的 window 活到那一轮真正结案为止——复核通过、结论落定，才轮到问能不能
关。中途关掉，等于把那份上下文连同复核能力一起扔了。
